...
?>
<div id="<?php  esc_attr_e( $this->token ); ?>_widget">
  <div class="SwitcherWrap">
    <div class="switcherBody">
              ...
              <?php 
                if(isset($this->acowcs_settings['curriencies']) && is_array($this->acowcs_settings['curriencies'])):
                  ...
                  foreach($this->acowcs_settings['curriencies'] as $k => $currency): 
                    ...
                    ?>
                    <option <?php esc_attr_e($selected); ?> value="<?php esc_attr_e($currency['currency']); ?>"><?php esc_attr_e(html_entity_decode($symbols[$currency['currency']]) . ' - ' . $currencies[$currency['currency']]); ?></option>
                    <?php
                  endforeach;
                endif;
              ?>
        ...
        <?php else: 
          $color = isset($this->acowcs_settings['shortcode_main_color']) && $this->acowcs_settings['shortcode_main_color'] != '' ? $this->acowcs_settings['shortcode_main_color'] : 'white';
          $bgcolor = isset($this->acowcs_settings['shortcode_bg_color']) && $this->acowcs_settings['shortcode_bg_color'] != '' ? $this->acowcs_settings['shortcode_bg_color'] : 'rgba(0, 0, 0, 0.8)';


          wp_add_inline_style(
            $this->token . '-widgetCSS', 
            '
                #acowcs_widget .SwitcherWrap .switcherBody nav ul li{
          ...
          <nav>   
                <ul>
                    ...
                    ?>
                        ...
                        $flagUrl = '';
                        if($show_flag && isset($currency['flag']) && $currency['flag'] != '') $flagUrl = $this->assets_url . 'images/flags/' . strtolower($currency['flag']) . '.svg';
